...
B <- 2500 ##--burn in
Total <- 5000
n <- dim(res_ag_ibd$post_c)[1]
Nsample <- dim(res_ag_ibd$post_c)[2] - B


####----save posterior samples after burn-in
c <- res_ag_ibd$post_c
gamma <- res_ag_ibd$post_gamma
alpha <- res_ag_ibd$post_alpha
lambda <- res_ag_ibd$post_lambda
...
partition_est[partition_est == 1] <- 6
partition_est[partition_est == 5] <- 7
...
p + geom_rect(data = data.frame(cluster = 1), aes(xmin = 0.5, xmax = 6.5, ymin = 32.5, ymax = 33.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  #geom_rect(data = data.frame(cluster = 1), aes(xmin = 0.5, xmax = 6.5, ymin = 57.5, ymax = 58.5), alpha = 0, color = "blue", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 1), aes(xmin = 0.5, xmax = 6.5, ymin = 30.5, ymax = 31.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  #geom_rect(data = data.frame(cluster = 1), aes(xmin = 0.5, xmax = 6.5, ymin = 14.5, ymax = 15.5), alpha = 0, color = "blue", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 2), aes(xmin = 0.5, xmax = 41.5, ymin = 32.5, ymax = 33.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 2), aes(xmin = 0.5, xmax = 41.5, ymin = 25.5, ymax = 27.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  #geom_rect(data = data.frame(cluster = 2), aes(xmin = 0.5, xmax = 41.5, ymin = 41.5, ymax = 42.5), alpha = 0, color = "blue", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 3), aes(xmin = 0.5, xmax = 73.5, ymin = 59.5, ymax = 61.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 3), aes(xmin = 0.5, xmax = 73.5, ymin = 68.5, ymax = 69.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 3), aes(xmin = 0.5, xmax = 73.5, ymin = 40.5, ymax = 41.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 4), aes(xmin = 0.5, xmax = 42.5, ymin = 73.5, ymax = 74.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 5), aes(xmin = 0.5, xmax = 27.5, ymin = 4.5, ymax = 5.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE) +
  geom_rect(data = data.frame(cluster = 5), aes(xmin = 0.5, xmax = 27.5, ymin = 6.5, ymax = 7.5), alpha = 0, color = "black", size = 0.7, inherit.aes = FALSE)
...
c4 = centroid$centroid[[4]]
c5 =centroid$centroid[[5]]


C = t(cbind(c1,c2,c3,c4,c5))
colnames(C) = colnames(y)
rownames(C) = c(1,2,3,4,5)


y <- t(otu_top)
ybar = apply(y, 1, sum)
y_ratio = sweep(y, 1, ybar, '/')
y_nmds = y


example_NMDS = metaMDS(y_nmds, k = 2)
...
color = c("blue", "red", "darkgoldenrod1", "green", "black", "black")
pch = c(15,16,17,18,19, 4)
...
plot(example_NMDS, display = "sites", type = "n", main = "DTMM", xlim = c(-0.7, 2.1))
points(example_NMDS, disp="sites", pch = mypch, col=mycol, bg = mycol, cex=0.8)
legend("topright", legend = c("cluster 1", "cluster 2", "cluster 3", "cluster 4", "cluster 5"), col = color,
       pch = pch, cex = 0.8)
...
color = c("red", "blue", "yellow", "green", "black", "yellow")
pch = c(16,15)
mycol = rep(0, length(r))
mypch = rep(0, length(r))
for(i in 1:length(mycol)){
...
plot(example_NMDS, display = "sites", type = "n", main = "DMM", xlim = c(-0.7, 2.1))
points(example_NMDS, disp="sites", pch = mypch, col = mycol, bg = mycol, cex=0.8)
...
partition_est_s[partition_est_s != 4] <- 1
...
comelt <- melt((C)^0.5)
comelt$cluster <- as.numeric(rep(c(1,2,3,4,5), 75))
head(comelt)
comelt$Var1 = factor(comelt$Var1)
comelt$Var2 = factor(comelt$Var2, levels = rev(colnames(y_sort)))
...
keeps <- c(" g__Bacteroides", " g__Ruminococcus", " g__Prevotella", " g__Faecalibacterium", " g__Blautia", " g__Enterobacter", " f__Rikenellaceae", " g__Proteus")
...
label_genus <- c("Enterobacter", "Ruminococcus", "Proteus", "Blautia", "Rikenellaceae*", "Prevotella", "Faecalibacterium", "Bacteroides")
...
q <- ggplot(data = comelt, mapping = aes(x=Var1, y=value, fill=Var2)) + geom_bar(position="stack", stat="identity") +
  scale_fill_brewer(palette="Dark2", name = "Genus", label = label_genus ) +
