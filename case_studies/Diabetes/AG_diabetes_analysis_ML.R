...
install.packages('clusteval', dependencies = TRUE) #Not available anymore
install.packages("G:/My Drive/R scripts/DTMM/ArchivedPackage/clusteval_0.1.tar.gz", repos = NULL, type = "source")

rm(list = ls())

setwd("G:/My Drive/R scripts/DTMM/DTMM_reproducible") #Set to my Google Drive folder
load("G:/My Drive/R scripts/DTMM/DTMM_reproducible/data/AG/AG_ibd.RData")
setwd("~/case_studies/Diabetes")
load(paste0(getwd(), "/case_studies/Diabetes/res_ag_diabetes.RData"))

library(DirichletMultinomial)
library(clusteval) # this package was removed from CRAN repository: https://cran.r-project.org/web/packages/clusteval/index.html
library(vegan)
library(ggplot2)
library(reshape2)
library(viridis)
library(parallel) #missing in the original script
...
tax = read.table(paste0(getwd(), "/data/AG/97_otu_taxonomy.txt"), sep = ";", header = FALSE, fill = TRUE, colClasses = "character")
...
centroid = HDTM_centroid(t(otu_top), tree, c = partition_est, gamma = gamma[,B + which.min(least_square) ]) #This functions is not available
centroid = DTMM_centroid(t(otu_top), tree, c = partition_est, gamma = gamma[,B + which.min(least_square) ])
#Error in HDTM_centroid(t(otu_top), tree, c = partition_est, gamma = gamma[,  : 
# could not find function "HDTM_centroid"
#
