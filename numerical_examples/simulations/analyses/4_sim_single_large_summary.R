...
result_n <- array(0, c(100, 7, 180))
result_s <- array(0, c(100, 7, 180))
result_m <- array(0, c(100, 7, 180))
result_l <- array(0, c(100, 7, 180))

for(i in 0:99){
  file <- paste("~/results/res_single_large_", i, ".RData", sep = "")
  load(file)
  result_n[i + 1, , ] <- res_single$clusters_n
  result_s[i + 1, , ] <- res_single$clusters_s
  result_m[i + 1, , ] <- res_single$clusters_m
  result_l[i + 1, , ] <- res_single$clusters_l
}
...
par(mfrow = c(2,2))
boxplot(t(jaccard_n), ylim = c(0.1, 1))
boxplot(t(jaccard_s), ylim = c(0.1, 1))
boxplot(t(jaccard_m), ylim = c(0.1, 1))
boxplot(t(jaccard_l), ylim = c(0.1, 1))
...
p <- ggplot(data, aes(x = Signal, y = Jaccard, fill = Method)) + facet_grid(~ Signal, switch = "x", scales = "free_x", space = "free_x") +
  geom_boxplot() + theme_bw() + scale_fill_brewer(palette="RdBu") + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + ggtitle("IV. LN-S")

#ggsave("sec_3_180_jac_lgnml_single.eps", plot = p, width = 6, height = 5.4, path = "~/plots")
