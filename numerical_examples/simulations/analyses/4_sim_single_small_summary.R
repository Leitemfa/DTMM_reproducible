...
for(i in 0:99){
  file <- paste("~/results/res_single_small_", i, ".RData", sep = "")
  load(file)
...
p <- ggplot(data, aes(x = Signal, y = Jaccard, fill = Method)) + facet_grid(~ Signal, switch = "x", scales = "free_x", space = "free_x") +
  geom_boxplot() + theme_bw() + scale_fill_brewer(palette="RdBu") + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + scale_y_continuous(limits=c(0.15,1)) + ggtitle("IV. LN-S")

#ggsave("sec_3_60_jac_lgnml_single.eps", plot = p, width = 6, height = 5.4, path = "~/plots")
