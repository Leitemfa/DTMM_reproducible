...
for(i in 0:99){
  file <- paste("~/results/res_dmm_large_", i, ".RData", sep = "")
  load(file)
  result_n[i + 1, , ] <- res_dmm$clusters_n
  result_s[i + 1, , ] <- res_dmm$clusters_s
  result_m[i + 1, , ] <- res_dmm$clusters_m
  result_l[i + 1, , ] <- res_dmm$clusters_l
}
...
p <- ggplot(data, aes(x = Signal, y = Jaccard, fill = Method)) + facet_grid(~ Signal, switch = "x", scales = "free_x", space = "free_x") +
  geom_boxplot( ) + theme_bw() + scale_fill_brewer(palette="RdBu") + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + scale_y_continuous(limits=c(0.15,1)) + ggtitle("II. Dir")

#ggsave("sec_3_180_jac_dmm.eps", plot = p, width = 6, height = 5.4, path = "~/plots")
